...
(function (w, o) {
    ...
    var shipTo = "";
    var sba_new = "";
    custTier = unescape(o.readCookie('customerTier') || '');
    ...
    var interval_orderNum = 10;
    var getTransactionalOrderNumber = function () {
        ...
        orderNumber = window.Analytics.tracking.orderconfirm.purchaseid;
      }
      else {
        interval_orderNum = interval_orderNum >= 1000 ? 1000 : interval_orderNum * 2;
        setTimeout(getTransactionalOrderNumber, interval_orderNum);
      }
    };

          if(localStorage.getItem("nc") != null){
            cartInfo = JSON.parse(localStorage.getItem("nc"));
            if(cartInfo.preTax != undefined){
              cartValue = cartInfo.preTax;
            }
			    if(cartInfo.total != undefined){
              cartValue = cartInfo.total;
            }
          }
          setTimeout(function(){
            if(transactionValue == "" || transactionValue == undefined){
                        if(window.Analytics && window.Analytics.tracking && window.Analytics.tracking.orderconfirm) {
                          transactionValue = window.Analytics.tracking.orderconfirm.orderrevenue;
                        }
                      }
                      if(cartValue == "" ){
                        if(window.Analytics && window.Analytics.global && window.Analytics.global.cart && (window.Analytics.global.cart.cartValue != undefined) && (_insideData.page.name != "Order Confirmation")){
                          cartValue = window.Analytics.global.cart.cartValue;
                        }
                      }
            },4000);
          if(OOo){
            jsessionid = OOo.readCookie("JSESSIONID");
            if(jsessionid == null || jsessionid == undefined || jsessionid == ""){
              jsessionid = OOo.readCookie("SASESSION");
            }
          }

          var tempurl = window.location.href;
          if (tempurl.indexOf("easycheckoutorderconf") != -1 || tempurl.indexOf("confirmOrder") != -1 && cartInfo != null) {
              transactionValue = (JSON.parse(localStorage.getItem("nc"))).preTax;
          }

          var _getZChatVisitorId = function () {
            if((typeof(window._insideGraph) != "undefined") && window._insideGraph.pid != "") {
              inside_id = window._insideGraph.pid;
              ZChatVisitorId = inside_id !== null ? inside_id.replace(/-(.*)/gm, '') : inside_id;
            }
            else {
              _interval = _interval >= 1000 ? 1000 : _interval * 2;
              setTimeout(_getZChatVisitorId, _interval);
            }
          };
          _getZChatVisitorId();
          getTransactionalOrderNumber();
          if(window.__PRELOADED_STATE__ && window.__PRELOADED_STATE__.headerState && window.__PRELOADED_STATE__.headerState.userInfo){
           userId = window.__PRELOADED_STATE__.headerState.userInfo.username;
    ...
    var OpinionLabInit = function () {
    	...
    	var currentHost = window.location.href,
    	    inside_ussandbox = OOo.readCookie('inside-ussandbox');



    	if (currentHost.indexOf("staplesadvantage") !== -1) {
	        var tCookie = o.readCookie('OSAJSESSIONID');

	        if (!!tCookie) {
	            tCookie = tCookie.split(':')[0];
	        } else {
	            tCookie = '';
	        }

	        o.oo_feedback = new o.Ocode({
	            tealeafCookieName:  'OSAJSESSIONID',
	            customVariables: {
	            	customerTier: unescape(o.readCookie('customerTier') || ''),
	            	ZChatVisitorId: ZChatVisitorId,
                ZCartValue: cartValue,
                ZSessionId : jsessionid,
                ZTransactionValue: transactionValue,
		        	transactionalOrderNumber: 'orderDetails' in this ?
			            		(!!orderDetails['info'] ?
			            				(!!orderDetails['info']['paragonOrderNumber'] ? orderDetails['info']['paragonOrderNumber'] : '' )
			            			: '' )
			            		: ''
	        	}
	        });

	        o.oo_feedback_yes = new o.Ocode({
	            referrerRewrite: {
	                searchPattern: /:\/\//g,
	                replacePattern: '://search.yes.'
	            },
	            tealeafCookieName: 'OSAJSESSIONID',
	            customVariables: {
	                tCookie: tCookie,
                  ...
                  ZSessionId : jsessionid,
                  ZTransactionValue: transactionValue,
	                AccountUserID: o.readCookie('companyIDuserID'),
	                ShipToLocation: o.readCookie('shipToName'),
	                transactionalOrderNumber: 'orderDetails' in this ?
	                		(!!orderDetails['info'] ?
	                				(!!orderDetails['info']['paragonOrderNumber'] ? orderDetails['info']['paragonOrderNumber'] : '' )
	                			: '' )
	        ...
	        o.oo_feedback_no = new o.Ocode({
	             referrerRewrite: {
	                 searchPattern: /:\/\//g,
	                 replacePattern: '://search.no.'
	             },
	             tealeafCookieName: 'OSAJSESSIONID',
	             customVariables: {
	                 tCookie: tCookie,
	        ...
	        o.oo_tab = new o.Ocode({
	            tab: {
	                ...
	                title: 'Feedback',
	                verbiage: 'Feedback',
	                iconPath: '/sbd/content/inc/voc/feedback/static/onlineopinionV5/'
	            },
	            tealeafCookieName:  'OSAJSESSIONID',
	            customVariables: {
	                AccountUserID: o.readCookie('companyIDuserID'),
	    ...
	    } else if (currentHost.indexOf("staples") !== -1) {
    ...
    var interval_custom = 10;
    var setCustomVariables = function () {
      var language = typeof langId !== "undefined" ? langId : null,
          tabText = "Feedback",
          languageRewrite = "";
      ...
      // Tealeaf Cookie Value
       tCookie = o.readCookie('JSESSIONID');
      if (!!tCookie) {
          tCookie = tCookie.split(':')[0];
      } else {
          tCookie = '';
      ...
      if((((typeof(window._insideGraph) != "undefined") && window._insideGraph.pid != "") && (window.Analytics && window.Analytics.tracking && window.Analytics.tracking.orderconfirm &&  window.Analytics.tracking.orderconfirm.purchaseid)) || (interval_custom > 5000)) {
        // instantiate Inline feedback object
        o.oo_feedback = new o.Ocode({
            legacyVariables: {
                vars: tCookie,
                override: true
            ...
            referrerRewrite: {
                searchPattern: /:\/\//g ,
                replacePattern: '://' + languageRewrite
            },
            customVariables: {
                mmxcart: ((OOo.readCookie('mmxcart') === null) && (OOo.readCookie('MMXCART') === null)) ? 'N' : 'Y',
                customerTier: custTier,
                ZChatVisitorId: ZChatVisitorId,
                ZCartValue: cartValue,
                ZSessionId : jsessionid,
                ZTransactionValue: transactionValue,
                transactionalOrderNumber: orderNumber,
                AccountUserID: userId,
                ShipToLocation: shipTo,
                SBA2019: ((OOo.readCookie('sba2019') === null) && (OOo.readCookie('SBA2019') === null)) ? 'N' : 'Y'
            }
        });

        // instantiate Inline Overlay object
        o.oo_inline_overlay = new o.Ocode({
            referrerRewrite: {
                searchPattern: /:\/\//g ,
                replacePattern: '://' + languageRewrite + 'overlay.'
            },
            legacyVariables: {
                vars: tCookie,
                override: true
        ...
        // instantiate Inline Overlay object
        o.oo_inline = new o.Ocode({
           referrerRewrite: {
               searchPattern: /:\/\/[^\/]*/,
               replacePattern: '://adfeedback.staples.com'
           },
           legacyVariables: {
               vars: tCookie,
               override: true
        ...
        // instantiate Tab feedback object
        o.oo_tab = new o.Ocode({
            tab: {
                  position: 'right',
                  title: 'Feedback',
        ...
        interval_custom = interval_custom >= 8000 ? 8000 : interval_custom * 2;
          setTimeout(setCustomVariables, interval_custom);
          if(!(window.Analytics && window.Analytics.tracking && window.Analytics.tracking.orderconfirm &&  window.Analytics.tracking.orderconfirm.purchaseid)) {
            getTransactionalOrderNumber();
          }
      }
    ...
    OOo.releaseDetails.push({
          author: 'AK',
          timeStamp: '01/16/2019, 18:23:11',
          fileName: 'staplesCombined.js',
          fileVersion: '1.0',
          ticketNumber: 'IMP-3556',
          gitDiff: 'N/A'
    });
...
function updateCartValue(){
  var _cartInfo = "";
  var _cartValue = "";
  var _transactionValue = ""
  if(localStorage.getItem("nc") != null){
    var _cartInfo = JSON.parse(localStorage.getItem("nc"));
    if(_cartInfo.preTax != undefined){
      _cartValue = _cartInfo.preTax;
    }
  if(_cartInfo.total != undefined){
      _cartValue = _cartInfo.total;
    }
  }
    if(_transactionValue == "" ){
      if(window.Analytics && window.Analytics.tracking && window.Analytics.tracking.orderconfirm) {
        _transactionValue = window.Analytics.tracking.orderconfirm.orderrevenue;
      }
    }
    if(_cartValue == "" ){
      if(window.Analytics && window.Analytics.global && window.Analytics.global.cart && (_insideData.page.name != "Order Confirmation") && (window.Analytics.global.cart.cartValue != undefined)){
        _cartValue = window.Analytics.global.cart.cartValue;
      }
    }
    if(window.OOo && window.OOo.oo_feedback && window.OOo.oo_feedback.options && window.OOo.oo_feedback.options.customVariables){
      OOo.oo_feedback.options.customVariables.ZCartValue = _cartValue;
    }
    if(window.OOo && window.OOo.oo_feedback && window.OOo.oo_feedback.options && window.OOo.oo_feedback.options.customVariables){
      OOo.oo_feedback.options.customVariables.transactionalOrderNumber = _transactionValue;
    }
}
